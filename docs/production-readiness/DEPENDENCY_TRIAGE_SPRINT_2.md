...
`npm audit --omit=dev` reports **5 HIGH, 0 CRITICAL**. None has a demonstrated exploitable path in the current Production or Browser Assistant request graph. This is an evidence-based mitigation, not a permanent waiver: the Next major and upstream Transformers/ONNX upgrades remain tracked on an isolated branch.
