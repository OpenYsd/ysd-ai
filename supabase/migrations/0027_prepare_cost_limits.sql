...
-- 0031 بعد أن ينشر التطبيق الجديد الذي يمرّ بعميل الخدمة.
--
-- **ترتيب الإصدار** موثّق في docs/RELEASE-v0.8.1.md، و**الترقيم يطابقه**:
--   (أ) 0027 ← هذه · ثم 0028 · ثم 0029 · ثم 0030   — كلها إضافية
--   (ب) التأكد أن التطبيق القديم ما زال يعمل
--   (ج) ضبط RATE_LIMIT_HMAC_SECRET في Railway
--   (د) نشر التطبيق الجديد
--   (هـ) فحص المحادثة والدعوات والحدود
--   (و) 0031 أخيرًا ← الوحيدة الكاسرة للقديم
--   (ز) التأكد أن anon يحصل على permission denied
--
-- الترقيم مقصودٌ أن يُطبَّق بالترتيب الطبيعي للأسماء: من يشغّل الترحيلات
-- تصاعديًا يحصل على الترتيب الصحيح بلا تعليمات جانبية. ولا يبقى إلا شرطٌ
-- واحد خارج الملفات: **النشر بين 0030 و0031**.
--
-- ── ما تفعله ──
--
