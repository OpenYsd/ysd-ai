...
describe("★ (٤) دليلُ التشغيل", () => {
  ...
  it("★ ★ ★ ولا يفترض أن البيئتين تُحدَّثان معًا", () => {
     ...
     */
    /**
     * ★ الحالةُ تتغيّر، والثابتُ لا.
     *
     * قيل «مُستعادٌ ومُتحقَّق» يوم بلغت دفعةٌ البيئتين — ثم كذّبته ثلاثُ
     * دفعاتٍ متتالية. فصار المكتوب جدولَ ما **لُوحظ** لا حكمًا عامًّا.
     *
     * والمحروسُ هنا ما لا يتغيّر بحالٍ: أن الوثيقة لا تَعِد بضمانٍ دائم،
     * وأنها تُلزم بفحص الوسم بعد كل دفعة. فالافتراض هو ما أخفى الانحراف
     * مرّتين — لا غيابُ الإصلاح.
     */
    expect(RUNBOOK).toMatch(/تحقّق بعد كل دفعة/);
    expect(RUNBOOK).toMatch(/railway status --json/);
    expect(RUNBOOK).toMatch(/ولا تفترض أن دفعةً وصلت/);
    /** ولا ضمانَ دائمًا بأنهما لن يفترقا */
    expect(RUNBOOK).not.toMatch(/فدفعةٌ واحدة تُحدّث الاثنتين معًا/);
    expect(RUNBOOK).not.toMatch(/لن تفترقا|يستحيل الافتراق/);
  });
  ...
  it("★ ★ ★ ويشرح كيف يُثبَت وصولُ التنبيه — ولا يدّعيه", () => {
    ...
    /**
     * ★ صار الوصولُ مُتحقَّقًا — والحارس يتبع الدليل لا العكس.
     *
     * شغّل المالك الاختبار، وفشل السير عمدًا، ووصله الإشعار. فلم يعد نفيُ
     * التحقّق صحيحًا.
     *
     * والمحروسُ الآن أدقّ: أن الادّعاء **لا يقف وحده**. فكلُّ حالةٍ
     * «مُتحقَّق» في هذه الوثيقة يجب أن تحمل معها ما يُثبتها — وإلا عادت
     * الوثيقة تَعِد بما لا دليلَ عليه.
     */
    expect(RUNBOOK).toMatch(/وصولُ التنبيه — \*\*مُتحقَّق\*\*/);
    expect(RUNBOOK).toMatch(/production-health workflow run failed/);
    expect(RUNBOOK).toMatch(/workflow_dispatch ⇒ failure/);
  });
...
describe("★ (٥) الحدود القائمة", () => {
  ...
  it("★ ★ ★ ولا ترحيلَ جديدًا في هذه المرحلة", () => {
     ...
     *  و0050 فهرسُ الجمل — كلُّها إضافيّةٌ محضة لا تحذف ولا تغيّر شيئًا قائمًا، ولكلٍّ حارسُه في
     *  v139-f2llm-migration و v141-file-pipeline-determinism و v146-migration-0050.)
     */
    ...
    expect(Math.max(...legacyNums)).toBe(50);
