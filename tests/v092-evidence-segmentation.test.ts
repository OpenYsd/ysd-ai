...
describe("★ (A–C) التوافق الخلفيّ لمحرّك v1", () => {
  const NUMBERED = "1. الأول\n2. الثاني\n3. الثالث";

  it("★ (A) الإصدار المفعَّل خادميًّا = 1 — المرحلة الثانية معطَّلة مؤقتًا", () => {
    /**
     * أُعيد إلى 1 حتى يُمرَّر الإصدار إلى مسار الاسترداد: هناك ما يزال
     * `parseEvidenceMarkers` يُستدعى بلا `segmentation`، فيفترق محرّكان
     * على قاعدة واحدة ويُلصق الدمجُ اقتباسًا بادّعاءٍ ليس له.
     */
    expect(SERVER_ENABLED_VERSION).toBe(1);
  });

  it("★ (B) v1 يُبقي البنود المرقّمة مقطعًا واحدًا — سلوك الرسائل القديمة", () => {
    const v1 = parseEvidenceMarkers(NUMBERED, { segmentation: 1 });
    /**
     * هذا هو العطل الأصل بعينه — ويبقى قائمًا عمدًا تحت v1.
     *
     * فالرسائل المولَّدة قبل المرحلة الثانية خُزِّن تخطيطها بـv1، وتغييرُ
     * محرّك v1 اليوم كان يزيح أزرارها عن مواضعها المحفوظة. الإصلاح يسري
     * على الجديد وحده.
     */
