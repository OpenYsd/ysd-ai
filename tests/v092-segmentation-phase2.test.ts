...
describe("★ (A–B) التفاوض والخادم عند 2", () => {
  it("★ التفعيل معطَّل مؤقتًا — الخادم عند 1 حتى يُصلَح الاسترداد", () => {
    /**
     * ★ هذا الاختبار يوثّق **ما هو مشحون**، لا ما نتمنّاه.
     *
     * قواعد v2 أدناه تُختبر بوسيط `segmentation: 2` صريح، فتبقى المحرّك
     * مُغطًّى كاملًا وإن لم يكن مفعَّلًا. وحين يُمرَّر الإصدار إلى مسار
     * الاسترداد يعود هذا السطر إلى 2 — وعندها يسقط هذا الاختبار عمدًا
     * فيُذكِّر بتحديثه، وهو المقصود.
     */
  ...
  it("★ (B) عميل يعلن 2 + خادم 2 ⇒ 2", () => {
    ...
    // ومع الخادم المشحون الآن (1) يهبط الاتفاق إلى 1 — وهو أثر التعطيل
    expect(negotiateSegmentationVersion(CLIENT_MAX_VERSION)).toBe(1);
  });
});
