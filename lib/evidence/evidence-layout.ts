 ...
 *
 * أُعيد إلى 1 مؤقتًا: مسار الاسترداد (`evidence-recovery`) ما يزال يحلّل
 * بـv1 لأن `parseEvidenceMarkers` تُستدعى فيه بلا `segmentation`. فبينما
 * كان الخادم عند 2 كان `resolved` ثلاثيّ المقاطع والاسترداد أُحاديّه —
 * ومطابقة الفهارس في الدمج تُلصق اقتباس ادّعاءٍ بادّعاءٍ آخر.
 *
 * والاستشهاد في غير موضعه أسوأ من غيابه، لأن الغياب يظهر للقارئ. فأُعيد
 * التفعيل إلى 1 حتى يُمرَّر الإصدار إلى الاسترداد، ثم يُرفع ثانيةً.
 *
 * والرسائل المخزَّنة بتخطيط v2 تبقى صحيحة العرض: كلٌّ تُعرض بتخطيطها هي
 * لا بإعادة حساب.
 */
export const SERVER_ENABLED_VERSION: SegmentationVersion = 1;
