 ...
 *
 * ── لماذا عشر دقائق لا تسعون ثانية ──
 *
 * كانت 90 ثانية، **وهي أقصر من الفاصل البشريّ بين الطلبين**: يقرأ المستخدم
 * الرد ثم يكتب التالي، فتنقضي المدة قبل وصوله. فيُشفى المزوّد ذاتيًّا، ثم
 * تُمنح رخصة التجربة الكاملة، فيعود القرار `healthy` بميزانية كاملة —
 * وسبرُ الست ثوانٍ لا يُبلَغ قطّ رغم أن التدهور وقع فعلًا.
 *
 * رُصد حيًّا بعد رقعة نافذة التجميع: طلبان فاشلان متتاليان بلغا العتبة
 * وضبطا `degradedUntil`، ثم بدأ الطلب التالي بـ`routing_decision=healthy`
 * و`primary_budget_ms=full`. الآلة فعلت ما صُمّمت له بالضبط — والخلل في
 * المعايرة لا في المنطق: مؤقّتٌ بالثواني في نظامٍ إيقاعه بالدقائق.
 *
 * وهذا ثاني ظهور للداء نفسه: `FAILURE_WINDOW_MS` عانته قبله.
 *
 * ── ولماذا خمس دقائق لا عشر ──
 *
 * **الترتيب مقصود: هذه أقصر من نافذة التجميع، لا مساوية لها.**
 *
 * فلو تساوتا لَوقع انقضاء التدهور بعد انقضاء التجميع أيضًا: تفشل التجربة
 * الكاملة، فيُصفَّر العدّاد لأن آخر فشل صار خارج النافذة، فيصير ١ لا ٢،
 * فلا يعود التدهور — ويمرّ **طلب ثانٍ** بالميزانية الكاملة قبل عودة السبر.
 * أي طلبان بطيئان لكل دورة بدل واحد.
 *
 * وبالفارق (٥ < ١٠) تقع التجربة الكاملة **داخل** نافذة التجميع: فشلها
 * يجد العدّاد ما يزال عند العتبة، فيعود التدهور فورًا والطلب التالي سبرٌ
 * من ست ثوانٍ. تجربةٌ واحدة كاملة لكل دورة — لا أكثر.
 *
 * ولا يُطيل ذلك حبس المزوّد: أول نجاح يشفيه فورًا (`recordProviderSuccess`)،
 * والرخصة تمنحه تجربةً كاملة كل دورة فلا يُهجَر بلا فرصة. وكلفة الخطأ سبرٌ
 * من ست ثوانٍ لا فشل — لأن الاحتياط يبقى خلفه.
 */
export const DEGRADED_WINDOW_MS = 300_000;
/**
 ...
 *
 * ── لماذا عشر دقائق لا دقيقة ──
 *
 * كانت 60 ثانية، **والطلب الفاشل نفسه يستغرق ~45 ثانية**: مهلتا «أول بايت»
 * متتاليتان في سلسلة OpenRouter تبلغان 40 ثانية وحدهما. فيكفي أن يفكّر
 * المستخدم ستّ عشرة ثانية بعد قراءة الرد حتى يتباعد الطلبان أكثر من
 * النافذة، فيُصفَّر العدّاد قبل بلوغ العتبة (٢).
 *
 * والأثر أن المزوّد لا يدخل التدهور أبدًا في الاستعمال البشري العادي:
 * `routing_decision=healthy` في كل طلب، وسبر الست ثوانٍ — وهو موجود ومُختبَر
 * — لا يُبلَغ قطّ. رُصد حيًّا: عنقود من خمس عيّنات بين 40687 و40944 مل، بمدى
 * 257 مل. ذلك التلاصق توقيع مهلٍ حتميّة لا تقلّب شبكة.
 *
 * والنافذة تجميعٌ لا حبس: هي تقول «متى يُعدّ الفشلان متتاليين»، لا «كم يبقى
 * المزوّد متدهورًا» — تلك `DEGRADED_WINDOW_MS`، وقد لحقت بها لاحقًا للسبب
 * نفسه: كلتاهما كانت أقصر من الإيقاع البشريّ. وتوسيع هذه لا يؤخّر
 * التعافي بحرف: أول نجاح يصفّر كل شيء فورًا (`recordProviderSuccess`).
 *
 * وحدّها الصريح: الحالة في ذاكرة العملية، فلا تعبر النسخ ولا إعادات التشغيل.
 * فأول طلبين بعد كل إقلاع يدفعان الكلفة كاملة — وذلك يحتاج حالة مشتركة، لا
 * توسيع نافذة.
 */
export const FAILURE_WINDOW_MS = 600_000;
