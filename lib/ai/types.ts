...
export interface StreamChunk {
  ...
  /**
   * ── نسب هدف YSD (v0.9.3) — حقول **خادمية داخلية** مع "meta" ──
   *
   * تجيب عن سؤالٍ تشغيليّ: «أيّ نسخةٍ وأيّ نشرةٍ أنتجتا هذا الرد؟» —
   * وهو سؤالٌ لا يُجاب بعد شهر إن لم يُلتقط لحظتَه، لأن النشرة تتغيّر.
   *
   * ★ ولا تُرسَل كلها إلى المتصفّح. المسار يلتقطها ويُبقيها عنده:
   * `modelVersion` وحدها تُحفظ مع الرسالة (نسبٌ مفيد للقارئ)، والمعرّفات
   * تذهب إلى الرصد الإداريّ وحده. فالمستخدم لا يعنيه أيّ نشرةٍ خدمته،
   * ومعرّفها تفصيلٌ تشغيليّ يكشف بنية النظام بلا فائدة له.
   *
   * و`model` يبقى على دلالته: النموذج الذي أجاب فعلًا.
   *
   * ★ وما لا يُضاف هنا أبدًا: `runtimeModel` و`artifactRef` و
   * `endpointAlias` و`baseUrl` و`apiKey`. تلك أهدافُ اتصالٍ وأسرار،
   * ووجودها في `StreamChunk` يجعل تسريبها سطرًا واحدًا لا قرارًا.
   */
  modelVersion?: string;
