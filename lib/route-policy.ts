...
export const PUBLIC_EXACT_PATHS: readonly string[] = [
  "/",
  /**
   * ★ مِنصّةُ قبولِ الاقتران — تطويرٌ فقط، **ولا تصل إلى الإنتاج**.
   *
   * صفحتُها تستدعي `notFound()` حين `NODE_ENV === "production"`، فالمسارُ
   * في الإنتاج يردّ 404 وكفى. ويُذكر هنا لأنّ حارسًا في
   * `tests/v121-public-failure-surfaces` يشترط أن تكون **كلُّ** صفحةٍ في
   * الشجرة مصنَّفةً صراحةً — فلا تُضاف صفحةٌ ويُنسى سطرُها.
   *
   * ★ ولا يفتح هذا شيئًا: أثرُه في الإنتاج أن يُردَّ 404 بدل تحويلٍ إلى
   *   `/login` — وذلك أقلُّ إفصاحًا لا أكثر.
   */
  "/local-pairing-harness",
];
